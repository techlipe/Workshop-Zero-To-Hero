    ...
    return new Promise(function (resolve, reject) {
        if (_this.db) {
            ...
            MongoClient.connect(uri, {
                ...
                function (database) {
                    logger.debug('connect database return', uri, database);
    ...
    if (this.db) {
        this.db.close()
            .then(
                ...
                function (error) {
                    logger.error("Failed to close the database: " + error.message)
                }
            );
    }
}
...
DB.prototype.countDocuments = function (coll) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        _this.dbo.collection(coll, function (error, collection) {
            if (error) {
                logger.error("Could not access collection: " + error.message);
                ...
                collection.count()
                    .then(
                        ...
                        function (err) {
                            logger.error("countDocuments failed: " + err.message);
        ...
        _this.dbo.collection(coll, function (error, collection) {
            if (error) {
                ...
                cursor.toArray(function (error, docArray) {
                    if (error) {
                        logger.error("Error reading fron cursor: " + error.message);
                        reject(error.message);
                    } else {
                        resolve(docArray);
                    }
    ...
    return new Promise(function (resolve, reject) {
        _this.dbo.collection(coll, function (error, collection) {
            if (error) {
                ...
                collection.updateMany(pattern, update, {
                        w: 1
                    })
                    .then(
                        function (result) {
                        ...
                        function (err) {
                            logger.error("updateMany failed: " + err.message);
                            reject(err.message);
                        }
                    )
            }
                ...
                if (!Array.isArray(docs)) {
                    ...
                    collection.insertMany(_docs)
                        .then(
                            ...
                            function (err) {
                                logger.error("Failed to insert Docs: " + err.message);
                                reject(err.message);
                            }
                        )
                }
...
DB.prototype.sumPoints = function (coll, initialDate, finalDate) {
    var _this = this;
    logger.debug('sumPoints start');
    return new Promise(function (resolve, reject) {
        _this.dbo.collection(coll, function (error, collection) {
            logger.debug('sumPoints collection open', initialDate, finalDate);
            if (error) {
                logger.error("Could not access collection: " + error.message);
                reject(error.message);
            } else {
                var cursor = collection.aggregate([{'$match': {date: {'$gte': initialDate, '$lte': finalDate}}},{'$group':{'_id': {'name': '$name'}, 'count': {'$sum': 1}}}]);
                cursor.toArray(function (error, docArray) {
                    if (error) {
                        logger.error("Error reading fron cursor: " + error.message);
                        reject(error.message);
...
DB.prototype.listPointsForMember = function (coll, initialDate, finalDate, memberName) {
    var _this = this;
    logger.debug('listPointsForMember start');
    return new Promise(function (resolve, reject) {
        _this.dbo.collection(coll, function (error, collection) {
            if (error) {
                logger.error("Could not acess collection: " + error.message);
                reject(error.message);
            } else {
                var cursor = collection.aggregate([{'$match': {date: {'$gte': initialDate, '$lte': finalDate}, name: memberName}}]);
                cursor.toArray(function (error, docArray) {
                    if (error) {
                        logger.error("Error reading fron cursor: " + error.message);
                        reject(error.message);
